...
type IndexerService struct {
	...
	backfiller ports.Backfiller
}

func NewIndexerService(repo ports.IndexerRepository, backfiller ports.Backfiller) *IndexerService {
	return &IndexerService{repo: repo, backfiller:backfiller}
}

func worker(ctx context.Context, chanJobs <-chan uint64, chanResults chan domain.BlockTxsEvents, backfiller ports.Backfiller) error {
	for {
		select {
		case <-ctx.Done(): {
...
func (service *IndexerService) Backfill(from uint64, concurrencyF int) error {
	...
	targetId, err := service.backfiller.GetLastBlockId()
	if err != nil {
		return err
	}
	...
	for range numWorkers {
		gWorkers.Go(func() error {
			return worker(ctxWorkers, chanJobs, chanResults, service.backfiller)
		})
	}
